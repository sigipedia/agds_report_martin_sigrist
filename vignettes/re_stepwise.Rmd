...
The aim of this chapter is stepwise forward regression. The dependent
variable is the gross primary production (GPP). It is defined a the
gross CO~2~ uptake flux driven by photosynthesis.
...
-   SWC\_\* (%): Volumetric soil water content
...
The stepwise forward regression algorithm is working. The implementation
would accept also a different number of independent variables. Future
improvements for an increased efficiency might be possible. The
computational effort was not a scope of this work, as the amount of data
to be processed is kept rather low.
The step-function from the stats-library gives the same results for the final linear model, R^2^ and BIC.
